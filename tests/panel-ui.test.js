...
test('panel UI includes docked-button, close, reset, and light-theme affordances', async () => {
  ...
  assert.match(download, /exportCurrentWorkToTxt/);
  assert.match(download, /htmlFor="fbe-export-format"/);
  assert.match(download, /id="fbe-export-format"/);
  assert.match(download, /<option value="fb2">FB2<\/option>/);
  assert.match(download, /<option value="txt">TXT<\/option>/);
  assert.match(download, /onClick=\{handleExport\}/);
  assert.match(download, /Скачать \$\{formatName\}/);
  assert.equal((download.match(/<button[^>]+fbe-export-button/g) || []).length, 1);
  assert.doesNotMatch(download, /fbe-export-button-secondary/);
  ...
  assert.doesNotMatch(styles, /fbe-export-button-secondary/);
  ...
  assert.match(exporter, /txt: \{ extension: 'txt', mimeType: 'text\/plain;charset=utf-8', serialize: serializeTxt \}/);
  assert.match(exporter, /export const exportCurrentWorkToTxt = \(options\) => exportCurrentWork\('txt', options\);/);
